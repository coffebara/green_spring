...
웹 어플리케이션을 좀 더 효율적으로 개발하고
개발한 이후 유지 보수를 쉽게 하기 위해 시스템 구조를 계층화하여 개발하는 것이 일반적임. 
도메인 객체(Domain Object)
 - 객체 정보를 저장하는 데이터 모델
퍼시스턴스 계층(Persistence Layer) = 데이터 엑세스 계층
 - 데이터베이스나 파일에 접근하여 데이터를처리하는 곳
서비스 계층(Service Layer) = 비지니스 계층
 - 애플리케이션이 제공하는 포괄적인 서비스들을 표현
 - 클라이언트에서 요청한 데이터를 가져오거나 변경하기 위해서 퍼시스턴스 계층을 호출하여,
 - 프레젠테이션 계츠ㅡㅇ과 퍼시스턴스 계층 사이의 연결하는 역할
프레젠테이션 계층(Presentation Layer)
 - 애플리케이션과 사용자와 최종 접점
 - 사용자로부터 데이터를 입력받거나 웹 서버로부터 데이터를 출력해 사용자에게 보이는 계층
 - 외부로부터 애플리케이션의 요청을 받아들여 처리하며 동시에 처리된 결과를  사용자에게 보여줌
 - 컨트롤러 -> 뷰
*/
public class Book {
